...
def process_images(
    ...
    images = [rescale(image , scale=rescale_factor) for
               image in images]
...
class PaliGemmaProcessor:
    ...
    def __init__(self, tokenizer, num_image_tokens: int, image_size: int):
        ...
        EXTRA_TOKENS += [
            f"<seg{i:04d}>" for i in range(1024)
        ]
    ...
    def __call__(
        ...
        assert len(images) == 1 and len(text) == 1, f" Received {len(images)} images and {len(text)} texts. This processor can only handle one image and one text at a time."
